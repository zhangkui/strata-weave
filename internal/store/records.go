...
func DispatchSample(db *sql.DB, id, lab string) error {
	r, e := db.Exec(`UPDATE samples SET status=?,lab_code=? WHERE id=? AND status=?`, model.SampleDispatched, lab, id, model.SampleCollected)
	if e != nil {
		return e
	}
	n, _ := r.RowsAffected()
...
func SubmitRecord(db *sql.DB, id string, at string) error {
	r, e := db.ExecContext(context.Background(), `UPDATE records SET status=?,submitted_at=? WHERE id=? AND status=?`, model.RecordSubmitted, at, id, model.RecordDraft)
	if e != nil {
		return e
	}
	n, _ := r.RowsAffected()
