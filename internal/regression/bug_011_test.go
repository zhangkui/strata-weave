...
func TestBug011_DispatchTransitionKeepsStateCause(t *testing.T) {
    app, _ := setup(t); find := reviewedFind(t, app); sample, err := app.CreateSample(context.Background(), model.Sample{ID: "s1", FindID: find.ID, Label: "charcoal"}); if err != nil { t.Fatal(err) }
    if err = app.DispatchSample(context.Background(), sample.ID, "Lab-A"); err != nil { t.Fatal(err) }
    err = app.DispatchSample(context.Background(), sample.ID, "Lab-A")
    if !errors.Is(err, model.ErrInvalidState) { t.Fatalf("invalid transition cause was lost: %v", err) }
}
