...
func TestBug012_ReviewValidationDoesNotClaimQueueItem(t *testing.T) {
    app, _ := setup(t); record, err := app.CreateRecord(context.Background(), model.FieldRecord{ID: "r1", UnitID: "u1", Author: "a", Notes: "record"}); if err != nil { t.Fatal(err) }
    if err = app.SubmitRecord(context.Background(), record.ID); err != nil { t.Fatal(err) }
    if err = app.ReviewRecord(context.Background(), record.ID, true, ""); !errors.Is(err, model.ErrInvalidInput) { t.Fatalf("empty note should be rejected: %v", err) }
    if err = app.ReviewRecord(context.Background(), record.ID, true, "verified stratigraphy"); err != nil { t.Fatalf("queue item was stranded after validation failure: %v", err) }
}
