...
func TestBug006_ConcurrentRelationsMustNotCloseCycle(t *testing.T) {
    app, _ := setup(t)
    var wg sync.WaitGroup
    for _, edge := range []model.Relation{{ID: "r12", EarlierID: "u1", LaterID: "u2"}, {ID: "r21", EarlierID: "u2", LaterID: "u1"}} {
        edge := edge; wg.Add(1); go func() { defer wg.Done(); _, _ = app.AddStratigraphicRelation(context.Background(), edge) }()
    }
    wg.Wait()
    relations, err := app.Matrix(context.Background()); if err != nil { t.Fatal(err) }
    if len(relations) != 1 { t.Fatalf("cycle protection accepted %d opposing relations", len(relations)) }
}
