...
func TestBug009_CanceledDispatchDoesNotLeaveSite(t *testing.T) {
    app, db := setup(t); find := reviewedFind(t, app); sample, err := app.CreateSample(context.Background(), model.Sample{ID: "s1", FindID: find.ID, Label: "charcoal"}); if err != nil { t.Fatal(err) }
    ctx, cancel := context.WithCancel(context.Background()); cancel()
    if err = app.DispatchSample(ctx, sample.ID, "Lab-A"); err == nil { t.Fatal("cancelled dispatch succeeded") }
    stored, err := store.GetSample(db, sample.ID); if err != nil { t.Fatal(err) }
    if stored.Status != model.SampleCollected { t.Fatalf("cancelled dispatch persisted %s", stored.Status) }
}
