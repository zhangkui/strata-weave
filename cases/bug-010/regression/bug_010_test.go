...
func TestBug010_MissingParentKeepsNotFoundCause(t *testing.T) {
    app, _ := setup(t)
    _, err := app.CreateSample(context.Background(), model.Sample{ID: "s1", FindID: "missing", Label: "charcoal"})
    if !errors.Is(err, model.ErrNotFound) { t.Fatalf("missing-find cause was lost: %v", err) }
}
