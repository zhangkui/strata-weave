...
func TestBug005_BatchAndSingleTelemetryShareLedger(t *testing.T) {
    app, _ := setup(t)
    batch := []model.Observation{observation("batch-1"), observation("batch-2")}
    var wg sync.WaitGroup
    wg.Add(2)
    go func() { defer wg.Done(); _, _ = app.IngestBatch(context.Background(), batch) }()
    go func() { defer wg.Done(); for n := 0; n < 50; n++ { _ = app.IngestObservation(context.Background(), observation(fmt.Sprintf("single-%d", n))) } }()
    wg.Wait()
}
