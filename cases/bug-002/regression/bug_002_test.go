...
func TestBug002_ReviewQueueConcurrentSubmission(t *testing.T) {
    app, _ := setup(t)
    record, err := app.CreateRecord(context.Background(), model.FieldRecord{ID: "r1", UnitID: "u1", Author: "a", Notes: "layer note"})
    if err != nil { t.Fatal(err) }
    var wg sync.WaitGroup
    for n := 0; n < 20; n++ { wg.Add(1); go func() { defer wg.Done(); _ = app.SubmitRecord(context.Background(), record.ID) }() }
    wg.Wait()
}
