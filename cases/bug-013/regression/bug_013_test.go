...
func TestBug013_ClosedAlertLeavesRuntimeFeed(t *testing.T) {
    app, _ := setup(t); alert, err := app.CreateAlert(context.Background(), model.Alert{ID: "a1", UnitID: "u1", Severity: "high", Message: "water ingress"}); if err != nil { t.Fatal(err) }
    if err = app.CloseAlert(context.Background(), alert.ID); err != nil { t.Fatal(err) }
    if active := app.ActiveRuntimeAlerts(); len(active) != 0 { t.Fatalf("closed alert remains in runtime feed: %#v", active) }
}
