...
func TestBug014_CollectedSampleDoesNotPanicWithoutDatingResult(t *testing.T) {
    app, _ := setup(t); find := reviewedFind(t, app)
    defer func() { if recovered := recover(); recovered != nil { t.Fatalf("sample creation panicked: %v", recovered) } }()
    if _, err := app.CreateSample(context.Background(), model.Sample{ID: "s1", FindID: find.ID, Label: "charcoal"}); err != nil { t.Fatal(err) }
}
