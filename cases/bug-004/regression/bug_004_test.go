...
func TestBug004_AlertLedgerConcurrentCreation(t *testing.T) {
    ...
    for n := 0; n < 24; n++ { wg.Add(1); go func(n int) { defer wg.Done(); _, _ = app.CreateAlert(context.Background(), model.Alert{ID: fmt.Sprintf("a-%d", n), UnitID: "u1", Severity: "high", Message: "water ingress"}) }(n) }
    wg.Wait()
}
