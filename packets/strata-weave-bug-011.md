# 出题包 strata-weave-bug-011

## 基本信息
| 字段 | 值 |
|---|---|
| session_id | <执行后填 UUID> |
| bug_id | strata-weave-bug-011 |
| task_type | diagnosis |
...
红测分支: bug11_red
基线提交: ebd3b6e952bc38c4ee8b498f90b99e8e934a6ad5
...
| 字段 | 值 |
|---|---|
| gold_root_cause | 中文根因：送检更新失败后，存储层与服务层连续使用未包装格式化，丢弃了无效状态的可判定语义。
生产文件/符号：`internal/store/records.go` 的 `DispatchSample`，以及 `internal/service/service.go` 的 `DispatchSample`。
调用链：DispatchSample -> store.DispatchSample -> SQLite 条件更新。
失效原因：调用者不能区分可重试的基础设施错误和重复送检的业务状态错误。
证据：第二次送检的回归用例无法识别 ErrInvalidState。 |
| gold_patch |  |

## 成功标准（success_criteria）
目标行为：诊断准确说明重复送检错误链的丢失点。
边界：不把已复核检查和实验室参数校验误判为根因。
合法场景：第一次送检保持可成功进入实验室链路。
验证标准：仅分析复现结果，不修改生产代码。

## docker 打包验证（阶段 5.5，交付前必跑）
- 构建：`./build_benzhi_docker.sh strata-weave-bug-11 linux/amd64`，再 `linux/arm64` 各一次（QEMU 模拟慢属正常）；**镜像名 `benzhi/strata-weave-bug-11:latest`（对齐 Go 项目打包规范模板）**
...
- <实测结果（按 bug 逐条，每 bug 一行）：bug-011：amd64 构建 OK / arm64 构建 OK / 容器内复现退出码 <与 success_criteria 一致>。少任何 bug = 未完成>
