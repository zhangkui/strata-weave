# 出题包 strata-weave-bug-007

## 基本信息
| 字段 | 值 |
|---|---|
| session_id | <执行后填 UUID> |
| bug_id | strata-weave-bug-007 |
| task_type | bugfix |
...
| 字段 | 值 |
|---|---|
| repo_url | https://github.com/zhangkui/strata-weave/tree/bug7_green |
| base_branch | 见上方绿测分支/红测分支/基线提交四行字段 |
| green_branch 语义 | **初始化时的未修复问题基线，不是修复完成代码** |
| agent workspace | **固定：`F:\skillxm12\strata-weave-bug-007-green`**（**执行端按需 clone** `bug7_green` 后 `git remote remove origin`；create 端不预建） |
| 准备命令 | 见下方详细步骤 |
...
```bash
git clone --single-branch -b bug7_green https://github.com/zhangkui/strata-weave.git F:\skillxm12/strata-weave-bug-007-green
cd F:\skillxm12/strata-weave-bug-007-green
...
## 用户需求（user_query）
现场网络中断后批量遥测导入的 context 已取消，但取消信号没有贯穿事务，仍可能写入观测数据。请修复批量导入的 context 生命周期传播。
...
```bash
go test ./cases/bug-007/regression -count=1 -run '^TestBug007_CanceledBatchDoesNotPersistTelemetry$'
```
...
目标行为：已取消的批量导入不写入任何遥测记录。
边界：未取消且格式合法的批量仍可整体保存。
合法场景：网络恢复后的新导入不受旧取消信号影响。
验证标准：修复后验证命令成功；修复前会发现取消后仍有持久化数据。

## docker 打包验证（阶段 5.5，交付前必跑）
- 构建：`./build_benzhi_docker.sh strata-weave-bug-7 linux/amd64`，再 `linux/arm64` 各一次（QEMU 模拟慢属正常）；**镜像名 `benzhi/strata-weave-bug-7:latest`（对齐 Go 项目打包规范模板）**
...
- <实测结果（按 bug 逐条，每 bug 一行）：bug-007：amd64 构建 OK / arm64 构建 OK / 容器内复现退出码 <与 success_criteria 一致>。少任何 bug = 未完成>
