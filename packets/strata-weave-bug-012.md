# 出题包 strata-weave-bug-012

## 基本信息
| 字段 | 值 |
|---|---|
| session_id | <执行后填 UUID> |
| bug_id | strata-weave-bug-012 |
| task_type | diagnosis |
| bug_category | 其他问题 |
...
绿测分支: bug12_green
红测分支: bug12_red
基线提交: e97e2a37f4fca4f090c303014aabee12d714ebbb

| 字段 | 值 |
|---|---|
| repo_url | https://github.com/zhangkui/strata-weave/tree/bug12_green |
| base_branch | 见上方绿测分支/红测分支/基线提交四行字段 |
| green_branch 语义 | **初始化时的未修复问题基线，不是修复完成代码** |
| agent workspace | **固定：`F:\skillxm12\strata-weave-bug-012-green`**（**执行端按需 clone** `bug12_green` 后 `git remote remove origin`；create 端不预建） |
| 准备命令 | 见下方详细步骤 |
...
```bash
git clone --single-branch -b bug12_green https://github.com/zhangkui/strata-weave.git F:\skillxm12/strata-weave-bug-012-green
cd F:\skillxm12/strata-weave-bug-012-green
...
## 用户需求（user_query）
请定位空复核意见为何会造成复核队列状态污染。不要修改代码，只分析验证、领取和完成步骤的调用顺序及后续记录被卡住的原因。
...
```bash
go test ./cases/bug-012/regression -count=1 -run '^TestBug012_ReviewValidationDoesNotClaimQueueItem$'
```

## 参考答案（整理端质检用，严禁交给执行模型）
| 字段 | 值 |
|---|---|
| gold_root_cause | 中文根因：空意见校验被放在队列领取之后，领取成功后失败路径未归还任务。
生产文件/符号：`internal/service/service.go` 的 `ReviewRecord`，以及 `internal/store/records.go` 的 `ReviewRecord`。
调用链：ReviewRecord -> ReviewQueue.Claim -> store.ReviewRecord -> ReviewQueue.Complete。
失效原因：首次无效复核占用队列项，后续有效复核无法再次领取。
证据：空意见失败后再次提交有效意见仍收到队列未找到错误。 |
| gold_patch |  |

## 成功标准（success_criteria）
目标行为：诊断准确说明队列状态污染的发生顺序。
边界：不把正常拒绝意见与队列领取错误混为一谈。
合法场景：有效意见能完成一次已提交记录的复核。
验证标准：仅分析复现结果，不修改生产代码。

## docker 打包验证（阶段 5.5，交付前必跑）
- 构建：`./build_benzhi_docker.sh strata-weave-bug-12 linux/amd64`，再 `linux/arm64` 各一次（QEMU 模拟慢属正常）；**镜像名 `benzhi/strata-weave-bug-12:latest`（对齐 Go 项目打包规范模板）**
...
- <实测结果（按 bug 逐条，每 bug 一行）：bug-012：amd64 构建 OK / arm64 构建 OK / 容器内复现退出码 <与 success_criteria 一致>。少任何 bug = 未完成>
