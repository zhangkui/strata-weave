# 出题包 strata-weave-bug-008

## 基本信息
| 字段 | 值 |
|---|---|
| session_id | <执行后填 UUID> |
| bug_id | strata-weave-bug-008 |
| task_type | bugfix |
...
绿测分支: bug8_green
红测分支: bug8_red
基线提交: 7de6b67c892b24d9347add63f637c7346ab068c4

| 字段 | 值 |
|---|---|
| repo_url | https://github.com/zhangkui/strata-weave/tree/bug8_green |
| base_branch | 见上方绿测分支/红测分支/基线提交四行字段 |
| green_branch 语义 | **初始化时的未修复问题基线，不是修复完成代码** |
| agent workspace | **固定：`F:\skillxm12\strata-weave-bug-008-green`**（**执行端按需 clone** `bug8_green` 后 `git remote remove origin`；create 端不预建） |
| 准备命令 | 见下方详细步骤 |
...
```bash
git clone --single-branch -b bug8_green https://github.com/zhangkui/strata-weave.git F:\skillxm12/strata-weave-bug-008-green
cd F:\skillxm12/strata-weave-bug-008-green
...
## 用户需求（user_query）
记录员撤销了现场记录提交请求，但 context 取消未被提交流程尊重，草稿可能仍被送入复核。请修复记录提交的取消传播和状态一致性。
...
```bash
go test ./cases/bug-008/regression -count=1 -run '^TestBug008_CanceledRecordSubmissionStaysDraft$'
```
...
目标行为：取消的提交保留草稿状态且不进入复核队列。
边界：有效未取消的草稿仍可正常提交一次。
合法场景：用户撤销慢网络请求后可以再次编辑记录。
验证标准：修复后验证命令成功；修复前记录会被错误提交。

## docker 打包验证（阶段 5.5，交付前必跑）
- 构建：`./build_benzhi_docker.sh strata-weave-bug-8 linux/amd64`，再 `linux/arm64` 各一次（QEMU 模拟慢属正常）；**镜像名 `benzhi/strata-weave-bug-8:latest`（对齐 Go 项目打包规范模板）**
...
- <实测结果（按 bug 逐条，每 bug 一行）：bug-008：amd64 构建 OK / arm64 构建 OK / 容器内复现退出码 <与 success_criteria 一致>。少任何 bug = 未完成>
