# 出题包 strata-weave-bug-005

## 基本信息
| 字段 | 值 |
|---|---|
| session_id | <执行后填 UUID> |
| bug_id | strata-weave-bug-005 |
| task_type | bugfix |
...
绿测分支: bug5_green
红测分支: bug5_red
基线提交: 74e6a8b2b759917991b94bdd5d851198f74c5201

| 字段 | 值 |
|---|---|
| repo_url | https://github.com/zhangkui/strata-weave/tree/bug5_green |
| base_branch | 见上方绿测分支/红测分支/基线提交四行字段 |
| green_branch 语义 | **初始化时的未修复问题基线，不是修复完成代码** |
| agent workspace | **固定：`F:\skillxm12\strata-weave-bug-005-green`**（**执行端按需 clone** `bug5_green` 后 `git remote remove origin`；create 端不预建） |
| 准备命令 | 见下方详细步骤 |
...
```bash
git clone --single-branch -b bug5_green https://github.com/zhangkui/strata-weave.git F:\skillxm12/strata-weave-bug-005-green
cd F:\skillxm12/strata-weave-bug-005-green
...
## 用户需求（user_query）
无人机批量导入观测数据时，记录员仍会补录单条高程，二者共享遥测账本并触发并发竞态。请修复批量导入与单条录入的实时数据访问。
...
```bash
go test -race ./cases/bug-005/regression -count=20 -run '^TestBug005_BatchAndSingleTelemetryShareLedger$'
```
...
目标行为：批量和单条观测并发执行时账本快照保持一致。
边界：批量内的无效行仍保留原有错误处理，单条数据不被覆盖。
合法场景：导入期间人工补测高程可安全保存。
验证标准：修复后竞态检测成功；修复前应稳定报告竞态。

## docker 打包验证（阶段 5.5，交付前必跑）
- 构建：`./build_benzhi_docker.sh strata-weave-bug-5 linux/amd64`，再 `linux/arm64` 各一次（QEMU 模拟慢属正常）；**镜像名 `benzhi/strata-weave-bug-5:latest`（对齐 Go 项目打包规范模板）**
...
- <实测结果（按 bug 逐条，每 bug 一行）：bug-005：amd64 构建 OK / arm64 构建 OK / 容器内复现退出码 <与 success_criteria 一致>。少任何 bug = 未完成>
