# 出题包 strata-weave-bug-009

## 基本信息
| 字段 | 值 |
|---|---|
| session_id | <执行后填 UUID> |
| bug_id | strata-weave-bug-009 |
| task_type | bugfix |
...
绿测分支: bug9_green
红测分支: bug9_red
基线提交: 3275d39e35e7862280824c053d6f33f3fc935496

| 字段 | 值 |
|---|---|
| repo_url | https://github.com/zhangkui/strata-weave/tree/bug9_green |
| base_branch | 见上方绿测分支/红测分支/基线提交四行字段 |
| green_branch 语义 | **初始化时的未修复问题基线，不是修复完成代码** |
| agent workspace | **固定：`F:\skillxm12\strata-weave-bug-009-green`**（**执行端按需 clone** `bug9_green` 后 `git remote remove origin`；create 端不预建） |
| 准备命令 | 见下方详细步骤 |
...
```bash
git clone --single-branch -b bug9_green https://github.com/zhangkui/strata-weave.git F:\skillxm12/strata-weave-bug-009-green
cd F:\skillxm12/strata-weave-bug-009-green
...
## 用户需求（user_query）
样本送检请求在离线切换时已取消，但 context 没有贯穿送检持久化，样本仍可能离开现场链路。请修复送检流程的取消传播。
...
```bash
go test ./cases/bug-009/regression -count=1 -run '^TestBug009_CanceledDispatchDoesNotLeaveSite$'
```
...
目标行为：取消的送检请求不会改变样本采集状态。
边界：已复核遗物的有效送检仍可进入实验室链路。
合法场景：取消后重新发起送检可正常完成。
验证标准：修复后验证命令成功；修复前样本会错误变为已送检。

## docker 打包验证（阶段 5.5，交付前必跑）
- 构建：`./build_benzhi_docker.sh strata-weave-bug-9 linux/amd64`，再 `linux/arm64` 各一次（QEMU 模拟慢属正常）；**镜像名 `benzhi/strata-weave-bug-9:latest`（对齐 Go 项目打包规范模板）**
...
- <实测结果（按 bug 逐条，每 bug 一行）：bug-009：amd64 构建 OK / arm64 构建 OK / 容器内复现退出码 <与 success_criteria 一致>。少任何 bug = 未完成>
