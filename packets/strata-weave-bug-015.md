...
| 字段 | 值 |
|---|---|
| repo_url | https://github.com/zhangkui/strata-weave/tree/bug15_green |
| base_branch | 见上方绿测分支/红测分支/基线提交四行字段 |
| green_branch 语义 | **初始化时的未修复问题基线，不是修复完成代码** |
| agent workspace | **固定：`F:\skillxm12\strata-weave-bug-015-green`**（**执行端按需 clone** `bug15_green` 后 `git remote remove origin`；create 端不预建） |
| 准备命令 | 见下方详细步骤 |
...
```bash
git clone --single-branch -b bug15_green https://github.com/zhangkui/strata-weave.git F:\skillxm12/strata-weave-bug-015-green
cd F:\skillxm12/strata-weave-bug-015-green
...
```bash
go test ./cases/bug-015/regression -count=1 -run '^TestBug015_UnreviewedFindCannotEnterLabChain$'
```
