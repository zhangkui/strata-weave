# 出题包 strata-weave-bug-004

## 基本信息
| 字段 | 值 |
|---|---|
| session_id | <执行后填 UUID> |
| bug_id | strata-weave-bug-004 |
| task_type | bugfix |
...
绿测分支: bug4_green
红测分支: bug4_red
基线提交: a85bf7cfead35ecbcc4a9abf6ad3e52c0d3b6dfb

| 字段 | 值 |
|---|---|
| repo_url | https://github.com/zhangkui/strata-weave/tree/bug4_green |
| base_branch | 见上方绿测分支/红测分支/基线提交四行字段 |
| green_branch 语义 | **初始化时的未修复问题基线，不是修复完成代码** |
| agent workspace | **固定：`F:\skillxm12\strata-weave-bug-004-green`**（**执行端按需 clone** `bug4_green` 后 `git remote remove origin`；create 端不预建） |
| 准备命令 | 见下方详细步骤 |
...
```bash
git clone --single-branch -b bug4_green https://github.com/zhangkui/strata-weave.git F:\skillxm12/strata-weave-bug-004-green
cd F:\skillxm12/strata-weave-bug-004-green
...
## 用户需求（user_query）
雨水渗入告警由多台现场设备同时创建，内存告警账本在并发合并时发生竞态。请修复告警持久化和实时告警视图的并发协作。
...
```bash
go test -race ./cases/bug-004/regression -count=20 -run '^TestBug004_AlertLedgerConcurrentCreation$'
```
...
目标行为：并发创建告警后实时告警视图完整且不发生竞态。
边界：重复编号仍受持久化唯一性约束，非法告警不可进入账本。
合法场景：多个探方同时上报高等级风险时均能记录。
验证标准：修复后竞态检测成功；修复前应稳定报告竞态。

## docker 打包验证（阶段 5.5，交付前必跑）
- 构建：`./build_benzhi_docker.sh strata-weave-bug-4 linux/amd64`，再 `linux/arm64` 各一次（QEMU 模拟慢属正常）；**镜像名 `benzhi/strata-weave-bug-4:latest`（对齐 Go 项目打包规范模板）**
...
- <实测结果（按 bug 逐条，每 bug 一行）：bug-004：amd64 构建 OK / arm64 构建 OK / 容器内复现退出码 <与 success_criteria 一致>。少任何 bug = 未完成>
